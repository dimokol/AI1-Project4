...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    for pair in itertools.combinations(literals, 2):
        paired_expr.append(disjoin(~(pair[0]), ~(pair[1])))
    return conjoin(*paired_expr)
    "*** END YOUR CODE HERE ***"
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    # Pacman's action at time t = 0
    KB.append(PropSymbolExpr(action0, time=0))
    # Pacman's action at time t = 1
    KB.append(PropSymbolExpr(action1, time=1))
    # Pacman's physics for time t = 0 and t = 1
    ...
    # Pacman is at (x1, y1) at time t = 1, Pacman is not at (x1, y1) at time t = 1
